...
function Teams() {
  return (
    ...
    lg:px-24 xl:px-40 pt-30 text-gray-800 dark:text-white'>
         ...
         <Title  title="Meet the team" description="A passionate team of digital exerts dedicated to your brands success." />
          
          <div className='grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-5'>
              {
                 teamData.map((team , index)=>(

                    <div className='flex max-sm:flex-col items-center gap-5 p-4 rounded-xl
                    border border-gray-100 dark:border-gray-700 bg-white dark:bg-gray-900
                    shadow-xl shadow-gray-100 dark:shadow-white/5 
                    hover:scale-105 transition-all duration-400' key={index}>
                        
                        <img src={team.image}   className='w-12 h-12 rounded-full' />

                        <div className='flex-1'>
                             <h3 className='font-bold-sm'>{team.name}</h3>
